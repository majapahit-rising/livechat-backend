...
app.post("/ai/chat", async (req, res) => {
    ...
    try {
        ...
        // ===============================
        // PERSIST TO DATABASE (chatbot_conversations)
        // ===============================
        const insertSql = `
        ...
        // ===============================
        // DETEKSI BYE/END DAN SIMPAN SESSION
        // ===============================
        const isEndingMessage = message.toLowerCase().includes('bye') || 
        ...
        // Jika ini adalah pesan penutup, simpan session summary
        if (isEndingMessage) {
            try {
                const sessionResult = await saveSessionSummary(
                    conversation_id, 
                    user_name, 
                    finalAgentType, 
                    message, 
                    n8nData.reply || "No response",
                    req.ip
                );
                
                console.log(`✅ Session summary saved for ${conversation_id}`);
                
                // Tambahkan flag untuk rating di response
                clientResponse.show_rating = true;
                clientResponse.session_ended = true;
                clientResponse.session_id = conversation_id;
                clientResponse.session_summary_id = sessionResult?.session_id;
                
            } catch (sessionErr) {
                console.error("❌ Failed to save session summary:", sessionErr);
            }
        }
...
// Helper function untuk extract search terms
function extractSearchTerms(message) {
   ...
   // Helper function untuk menyimpan session summary
      async function saveSessionSummary(sessionId, userName, agentType, lastUserMessage, lastAiResponse) {
          return new Promise((resolve, reject) => {
              // 1. Hitung statistik dari conversations untuk session ini
              const statsQuery = `
                  SELECT 
                      COUNT(*) as total_messages,
                      SUM(CASE WHEN user_message IS NOT NULL THEN 1 ELSE 0 END) as user_messages,
                      SUM(CASE WHEN ai_response IS NOT NULL THEN 1 ELSE 0 END) as ai_messages,
              ...
              db.query(statsQuery, [sessionId], (err, results) => {
                  if (err) {
                  ...
                  const endedAt = stats.ended_at || new Date();
                  
                  // Hitung durasi session
                  const duration = startedAt && endedAt ? 
                      Math.floor((new Date(endedAt) - new Date(startedAt)) / 1000) : 0;
      
                  // 2. Ambil conversation summary (5 pesan terakhir)
                  const summaryQuery = `
                  ...
                  db.query(summaryQuery, [sessionId], (summaryErr, conversations) => {
                      if (summaryErr) {
                          console.error("❌ Error getting conversations:", summaryErr);
                          return reject(summaryErr);
                      }
      
                      // Buat summary text
                      let summary = `Session with ${userName} (Agent: ${agentType})\n\n`;
                      conversations.reverse().forEach((conv, index) => {
                          if (conv.user_message) {
                              summary += `User: ${conv.user_message.substring(0, 100)}\n`;
                          }
                          if (conv.ai_response) {
                              summary += `AI: ${conv.ai_response.substring(0, 150)}\n\n`;
                          }
                      });
      
                      // 3. Insert ke chatbot_conversation_sessions
                      ...
                      const sessionIdUnique = `session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
                      const conversationId = sessionId; // Gunakan session_id sebagai conversation_id
      
                      db.query(insertSessionQuery, [
                          sessionIdUnique,          // session_id (unik untuk table ini)
                          conversationId,          // conversation_id (sama dengan session_id dari conversations)
                          null,                    // system_type_id
                          null,                    // customer_id
                          null,                    // lead_id
                          null,                    // user_email
                          ...
                          0,                       // escalated_to_human
                          null,                    // escalation_reason
                          summary.substring(0, 2000), // conversation_summary (max 2000 chars)
                          null,                    // ai_rating
                          null,                    // ai_feedback
                          ...
                          console.log(`✅ Session saved with ID: ${insertResult.insertId}`);
                          resolve({
                              session_id: sessionIdUnique,
       ...
       app.post("/ai/save-rating", (req, res) => {
          ...
          const ratingEnum = ratingMap[rating] || 'neutral';
      
          // Update rating di chatbot_conversation_sessions
          const updateQuery = `
          ...
          db.query(updateQuery, [ratingEnum, feedback, session_id, conversation_id], (err, result) => {
              if (err) {
              ...
              // Juga update di chatbot_conversations (opsional)
              if (conversation_id) {
                  const satisfactionMap = {
                      'very_poor': 'not_helpful',
                      'poor': 'not_helpful',
                      'neutral': 'neutral',
                      'good': 'helpful',
                      'excellent': 'helpful'
                  ...
                  db.query(`
                      ...
                      ORDER BY created_at DESC 
                      LIMIT 1
                  `, [satisfactionMap[ratingEnum], conversation_id]);
              }
